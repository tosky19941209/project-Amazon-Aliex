...
status_qoo_buttonclick_count = 0
...
class WorkerThread(QThread):
    ...
    def run(self):
        ...
        # if state_flag == 1:
        #     Amazon_products(1)
        # BGMainUi.ConnectServer_amazon()
        Amazon_products(self.flag)
        time.sleep(3)
        self.finished.emit()

class WorkThread_Draw(QThread):
    progress_load_image = pyqtSignal(QtGui.QPixmap, str, str, str, str, int)
    # progress_load_image = pyqtSignal(str, str, str, str, int)
    ...
    def run(self):
        ...
        for i in range(1, count):
            ...
            if self.filename =='消去.csv':
                response = str(result[0][i])
                image = QtGui.QPixmap()
                # image.loadFromData(BytesIO(response.content).read())
                img_product = QtGui.QPixmap(image)   

            title = str(result[1][i])

            price = str(result[2][i])
            ...
            # self.progress_load_image.emit( title, price, item_qty, item_asin, i)
...
class BGMainUi(QtWidgets.QWidget):
    ...
    def btn_event(self):
        # self.ui.btn_get2.hide()
        self.ui.btn_compare.clicked.connect(lambda: self.compare_amazon(1))
    ...
    def on_thread_get_finished(self):
        self.ui.btn_get.setText('新しく 抽出')
        self.ui.label_loading.hide()
   ...
   # Run the dialog, and check results   
    ...
    def img_product(self, img_source):
        img_source = img_source.scaled(80, 80)
        img_product = QtWidgets.QLabel()
        img_product.setPixmap(img_source)
        return img_product
    def compare_amazon(self, flag):
        self.thread_compare = WorkerThread(flag)
        self.thread_compare.finished.connect(self.on_thread_compare_finished)
        self.thread_compare.start()
        self.ui.btn_compare.setText("作業中")
        self.ui.label_loading.show()
    def on_thread_compare_finished(self):
        self.ui.label_loading.hide()
        self.ui.btn_compare.setText("抽出 比較")
        self.ConnectServer_amazon('Amazon_製品(2).csv')
    def set_rate(self):
    ...
    def send_updatedData(self):
        # from Qoo10.Update_CSV import update_csv
        global status_market_qoo, status_market_amazon

        # main_id('qoo', self.key, self.userid, self.password)
